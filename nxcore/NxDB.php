...
class NxDB extends BaseObject
{
	...
	// 对于 MySQL, MariaDB, MSSQL, Sybase, PostgreSQL, Oracle，有服务器主机选项
	protected $server;
	...
	// 对于 MySQL/MariaDB 使用 unix_socket
	protected $socket;

	// 对于 SQLite，如果没有安装MySQL之类的数据库，可考虑使用 SQLite
	...
	{
		$dbconf = require NX_ROOT.'/config/db.php';
		try {
			$commands = array();
			$dsn = '';
			...
			if (is_array($dbconf))
			{
				foreach ($dbconf as $option => $value)
				{
					$this->$option = $value;
				}
			}
			...
			{
				$this->db_prefix = $dbconf[ 'db_prefix' ];
			}
			...
			switch ($type)
			{
				...
				case 'oracle':
					...
					$dsn = 'oci:dbname=' . $dbname . ($this->db_charset ? ';charset=' . $this->db_charset : '');
					break;
			...
			{
				$commands[] = "SET NAMES '" . $this->db_charset . "'";
			}
	...
	// 调试模式设置
	public function debug($is_debug = true)
	{
